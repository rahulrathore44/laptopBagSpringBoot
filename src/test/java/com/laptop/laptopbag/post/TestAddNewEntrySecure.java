...
public class TestAddNewEntrySecure {
	...
	public void test_post_with_json_data_accept_json() throws Exception {
		Gson gson = new GsonBuilder().setPrettyPrinting().setFieldNamingPolicy(FieldNamingPolicy.UPPER_CAMEL_CASE)
				.create();
		String jsonContent = gson.toJson(getLaptopWithRandomId());
		ResultActions response = mvc.perform(post(contextPath + "/add").headers(getBasicAuthHeader())
				.accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_JSON).content(jsonContent));
	...
	public void test_post_with_xml_data_accept_xml() throws Exception {
		...
		ResultActions response = mvc.perform(post(contextPath + "/add").headers(getBasicAuthHeader())
				.accept(MediaType.APPLICATION_XML).contentType(MediaType.APPLICATION_XML).content(xmlContent));
	...
	public void test_post_with_json_data_accept_xml() throws Exception {
		...
		ResultActions response = mvc.perform(post(contextPath + "/add").headers(getBasicAuthHeader())
				.accept(MediaType.APPLICATION_XML).contentType(MediaType.APPLICATION_JSON).content(jsonContent));
	...
	public void test_post_with_xml_data_accept_json() throws Exception {
		...
		ResultActions response = mvc.perform(post(contextPath + "/add").headers(getBasicAuthHeader())
				.accept(MediaType.APPLICATION_JSON).contentType(MediaType.APPLICATION_XML).content(xmlContent));
